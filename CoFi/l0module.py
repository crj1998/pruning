...
class L0Module(nn.Module):
    def __init__(self,
        ...
        self.lambda_1 = nn.Parameter(torch.tensor(0.01))
        self.lambda_2 = nn.Parameter(torch.tensor(0.01))
        ...
        for type in self.types:
            logger.info(f'{type}: z.shape {tuple(self.z_logas[type].shape)} size {self.sizes[type]}')
        logger.info(f"prunable model size: {self.prunable_model_size}")

    def set_lagrangian_warmup_steps(self, lagrangian_warmup):
        self.lagrangian_warmup = int(lagrangian_warmup)
    ...
    def lagrangian_regularization(self, pruned_steps) -> Tuple[Optional[torch.Tensor], Optional[torch.Tensor], Optional[torch.Tensor]]:
    ...
    def get_z_from_zs(self, zs):
        ...
        for t in self.all_types:
            name = t[:-2]
            z = zs.get(t, np.ones(self.shapes[name]))
            if torch.is_tensor(z): 
                new_z = z.squeeze().detach().cpu().numpy() > 0.0
...
if __name__ == "__main__":
    ...
    # print(config)
    # l0_module = torch.load("/root/rjchen/workspace/CoFi/l0_module.pt")
    # print(l0_module)
